...
const getServicesTool = tool({
  description:
    'Get list of dental services with prices. Use when user asks about services, treatments, or prices.',
  inputSchema: z.object({
    category: z
      .string()
      .nullable()
      .describe(
        'Optional category filter: Діагностика, Гігієна, Терапія, Хірургія, Імплантологія, Протезування, Естетика, Ортодонтія, Дитяча стоматологія'
      ),
  }),
...
const getDoctorsTool = tool({
  description:
    'Get information about clinic doctors. Use when user asks about doctors, specialists, or team.',
  inputSchema: z.object({
    specialization: z
      .string()
      .nullable()
      .describe('Optional specialization filter'),
  }),
...
const getWorkingHoursTool = tool({
  description:
    'Get clinic working hours. Use when user asks about schedule, opening hours, or when the clinic is open.',
  inputSchema: z.object({}),
  execute: async () => {
    const supabase = await createClient()
    if (!supabase) return { error: 'Service unavailable' }
...
const analyzeSymptomsTool = tool({
  description:
    'Analyze dental symptoms and provide preliminary assessment. Use when user describes pain, discomfort, or dental issues.',
  inputSchema: z.object({
    symptoms: z
      .array(z.string())
      .describe('List of symptoms described by user'),
    painLevel: z
      .number()
      .min(1)
      .max(10)
  ...
  execute: async ({ symptoms, painLevel }) => {
    ...
    const symptomAnalysis: Record<
      string,
      { conditions: string[]; urgency: string; recommendation: string }
    > = {
      біль: {
        conditions: ['Карієс', 'Пульпіт', 'Періодонтит'],
        urgency: 'середня',
        recommendation: 'Рекомендуємо записатися на огляд протягом 1-2 днів',
      },
      набряк: {
        conditions: ['Абсцес', 'Періодонтит', 'Перикороніт'],
        urgency: 'висока',
        recommendation: 'Потрібна термінова консультація!',
      },
      кровоточивість: {
        conditions: ['Гінгівіт', 'Пародонтит'],
        urgency: 'низька',
        recommendation: 'Рекомендуємо професійну чистку та огляд пародонтолога',
      },
      чутливість: {
        conditions: ['Карієс', 'Оголення шийки зуба', 'Тріщина емалі'],
        urgency: 'низька',
        recommendation: 'Запишіться на діагностику',
      },
      запах: {
        conditions: ['Карієс', 'Захворювання ясен', 'Зубний камінь'],
        urgency: 'низька',
        recommendation: 'Рекомендуємо професійну чистку',
      },
    }
    ...
    const findings = symptoms.map(s => {
      ...
      return {
        symptom: s,
        conditions: ['Потребує огляду'],
        urgency: 'середня',
        recommendation: 'Рекомендуємо консультацію спеціаліста',
      }
    })
    ...
    return {
      ...
      nextStep:
        hasHighUrgency || highPain
          ? `Телефонуйте нам негайно: ${CONTACT_INFO.phone}`
...
const bookingHelperTool = tool({
  description:
    'Help user with booking information. Use when user wants to make an appointment.',
  inputSchema: z.object({
    serviceType: z
      .string()
      .nullable()
      .describe('Type of service user is interested in'),
    preferredDate: z
      .string()
      .nullable()
  ...
  execute: async ({ serviceType, preferredDate }) => {
    return {
      ...
      note: serviceType
        ? `Для послуги "${serviceType}" рекомендую попередню консультацію`
        : null,
...
const systemPrompt = `Ви - віртуальний асистент стоматологічної клініки "${SITE_INFO.name}" у Львові, Україна.
...
7. Відповідь показується у вузькому чат-вікні як звичайний текст: БЕЗ Markdown-розмітки (жодних ###, **, таблиць, блоків коду). Пункти списку починайте з «— ». Відповідайте стисло; довгі переліки скорочуйте до найзапитуваніших позицій і пропонуйте уточнити

КОНТАКТНА ІНФОРМАЦІЯ:
- Телефон: ${CONTACT_INFO.phone}
- Адреса: ${CONTACT_INFO.address.full}
