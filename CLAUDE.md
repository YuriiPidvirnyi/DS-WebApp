...
2. ESLint bans hardcoded Cyrillic literals in JSX outside i18n. The stock module and the remaining legacy pages (auth flow, payment result, cabinet error/payments, About, WalletCards, MonoPayButton, admin materials/email-templates) are fully internationalized and enforced. The cabinet payments page keeps its server component for auth + data fetch (`app/cabinet/payments/page.tsx`, no user-facing copy) and renders a client view (`src/views/cabinet/CabinetPaymentsPage.tsx`) that goes through `t()` — the standard server-fetch → client-render split. `app/global-error.tsx` replaces the root layout (no i18n provider **or** Tailwind in scope), so it stays deliberately dependency-free and localizes via a self-contained inline uk/en/pl dictionary keyed off the persisted `i18nextLng` language — it's enforced, not exempt. Only print/promo documents (intentionally Ukrainian clinical papers) stay exempt.
