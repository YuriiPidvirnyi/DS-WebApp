...
export default function ReminderSettings() {
  ...
  const handlePreferenceChange = async (
    ...
    try {
      await withToast(
        async () => {
          const response = await updateReminderPreference(
            appointmentId,
            preference
          )
          if (!response.success) {
  ...
  const handleCalendarDownload = (reminder: ScheduledReminder) => {
    try {
      ...
      if (lastBookingJson) {
        ...
        if (
          booking.id === reminder.appointmentId &&
          booking.date &&
          booking.time
        ) {
  ...
  const reminderGroups = reminders.reduce<Record<string, ScheduledReminder[]>>(
    (acc, reminder) => {
                    ...
                    <div className="flex gap-2">
                      {(['email', 'sms', 'both', 'none'] as const).map(pref => (
                        <button
                          key={pref}
                          onClick={() =>
                            handlePreferenceChange(appointmentId, pref)
                          }
