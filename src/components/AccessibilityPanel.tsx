    ...
    <div className="fixed bottom-4 left-4 z-50" role="region" aria-label="Налаштування доступності">
      ...
      {isOpen && (
        ...
        >
          ...
          {/* Font size controls */}
          <div className="mb-4">
            <h4 className="text-sm font-medium text-gray-700 mb-2">Розмір тексту</h4>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-500 ml-2">
                {fontSize === 'normal' ? 'Стандартний' : fontSize === 'larger' ? 'Великий' : 'Найбільший'}
              </span>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="relative inline-block w-10 align-middle select-none">
                <input 
                  ...
                  className="sr-only"
                />
                <div className={`block h-6 rounded-full w-10 transition-colors duration-200 ease-in-out ${highContrast ? 'bg-dental-teal' : 'bg-gray-300'}`}></div>
                <div className={`absolute left-0.5 top-0.5 bg-white border w-5 h-5 rounded-full transition-transform duration-200 ease-in-out transform ${highContrast ? 'translate-x-4 border-dental-teal' : 'translate-x-0 border-gray-300'}`}></div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="relative inline-block w-10 align-middle select-none">
                <input 
                  ...
                  className="sr-only" 
                />
                <div className={`block h-6 rounded-full w-10 transition-colors duration-200 ease-in-out ${reducedMotion ? 'bg-dental-teal' : 'bg-gray-300'}`}></div>
                <div className={`absolute left-0.5 top-0.5 bg-white border w-5 h-5 rounded-full transition-transform duration-200 ease-in-out transform ${reducedMotion ? 'translate-x-4 border-dental-teal' : 'translate-x-0 border-gray-300'}`}></div>
              </div>
            </div>
          </div>
          ...
          <div>
            <h4 className="text-sm font-medium text-gray-700 mb-2">Режим сприйняття кольорів</h4>
            <select 
              value={colorBlindnessMode} 
              onChange={(e) => setColorBlindnessMode(e.target.value as 'normal' | 'protanopia' | 'deuteranopia' | 'tritanopia')} 
