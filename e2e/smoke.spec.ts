...
test.describe('Smoke', () => {
  test('home -> nav works', async ({ page }) => {
    ...
    await page
      .getByRole('navigation', { name: 'Основна навігація' })
      .getByRole('link', { name: 'Про нас', exact: true })
      .click()
    await expect(page).toHaveURL(/\/about$/)

    await page
      .getByRole('navigation', { name: 'Основна навігація' })
      .getByRole('link', { name: 'Контакти', exact: true })
      .click()
  ...
  test('contact form submits (mock)', async ({ page }) => {
    ...
    await page
      .getByLabel(/Повідомлення/i)
      .fill('Тестове повідомлення щонайменше 10 символів')
    await page.getByLabel(/Я даю згоду/i).check()

    await page.getByRole('button', { name: /Надіслати повідомлення/i }).click()

    await expect(page.getByText(/Повідомлення успішно надіслано/i)).toBeVisible(
      { timeout: 10000 }
    )
  })
  ...
  test('booking flow (mock)', async ({ page }) => {
    ...
    await page.route('**/appointments', async route => {
      if (route.request().method() === 'POST') {
        ...
        await route.fulfill({
          ...
          body: JSON.stringify({
            success: true,
            data: { id: 'apt-e2e', status: 'pending', createdAt: now },
          }),
        })
        return
      }
    ...
    await page
      .locator('select[name="time"]')
      .first()
      .selectOption({ label: '10:00' })
    ...
    await page
      .locator('form')
      .first()
      .locator('input[type="checkbox"][name="consent"]')
      .check()
    ...
    const ensuredRef = await page.evaluate(d => {
      try {
        ...
        localStorage.setItem(
          ...
          JSON.stringify({
            id,
            service: 'Терапевтична стоматологія',
            date: d,
            time: '10:00',
