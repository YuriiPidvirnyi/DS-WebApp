...
if (!siteUrl || !startDate || !endDate) {
  ...
  console.error(
    'Example: scripts/seo/search-console-export.mjs https://example.com 2024-01-01 2024-01-31'
  )
  process.exit(1)
}
...
if (!token) {
  ...
  console.error(
    'Create an OAuth 2.0 access token with Search Console API scope:'
  )
  console.error('  https://www.googleapis.com/auth/webmasters.readonly')
  console.error('Then run:')
  console.error(
    '  GSC_ACCESS_TOKEN=... scripts/seo/search-console-export.mjs <siteUrl> <start> <end>'
  )
  process.exit(1)
}
